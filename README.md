...
已实现首批 MCU 诊断工具，可用 `airbornediag diag examples/REC-2026-0918-002.json` 读取故障记录并按固定规则分析其中的已解码位域，输出确认的状态、输入中的矛盾、因缺失而无法判定的内容以及本版不支持的范围；每条结论都带手册出处与对应的知识条目。工具覆盖 MPC5554 的 FlexCAN2（`CAN_x`）与 DSPI（`DSPI_x`），可脱离模型服务独立运行，也有便于机器读取的 `--json` 输出。**工具只使用记录中已解码的位域，只给出原始值的观测不参与判断；候选根因、知识检索与完整诊断报告尚未接入。**
