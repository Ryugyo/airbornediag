...
已建立首批 MCU 诊断知识库并实现全文检索。知识条目整理自 MPC5553/5554 参考手册，覆盖 MPC5554 的 FlexCAN2 与 DSPI，每条保留来源、章节与表/图编号；检索用 Python `sqlite3` 的 FTS5 实现，可用 `airbornediag kb build` 构建索引、`airbornediag kb query "总线关闭"` 查询，支持按芯片和外设过滤、限制返回条数。索引为本地生成物，不提交仓库，在目标机器上重建。已在 Windows 与 RDC300I 上验证构建与检索结果一致。**知识来源为芯片手册原文；寄存器解析、诊断规则与报告生成尚未实现。**
