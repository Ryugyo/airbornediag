...
3. 场景判据、支持范围与明确不支持的结论见 [scenarios.md](scenarios.md)。诊断工具只使用记录中**已解码的位域**：位编号尚未逐条核对入知识库，因此只给出 `value`、没有 `fields` 的观测不会被按位解释，而是由工具报告为不支持，契约侧也不为其提供解析依据。
4. **已观测但没有判据不等于未观测。** 位域已解码、本版却还没有对应判据时，工具按「已观测但没有判据」列出并附上观测 id，与「记录中未提供该观测」是两种不同的报告，不得互相误读；两者都不参与判断，也都不构成正常或检测通过的依据。
